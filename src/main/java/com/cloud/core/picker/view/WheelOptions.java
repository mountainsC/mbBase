package com.cloud.core.picker.view;
...
import com.cloud.core.picker.adapter.ArrayWheelAdapter;
import com.cloud.core.picker.lib.WheelView;
import com.cloud.core.picker.listener.OnItemSelectedListener;
...
public class WheelOptions<T> {
    ...
    private OnItemSelectedListener wheelListener_option1 = new OnItemSelectedListener() {
        ...
        public void onItemSelected(int index) {
            try {
                ...
                if (mOptions2Items != null) {
                    ...
                    if (mOptions2Items.size() > index) {
                        List<T> list = mOptions2Items.get(index);
                        opt2Select = opt2Select >= list.size() - 1 ? list.size() - 1 : opt2Select;
    ...
    private OnItemSelectedListener wheelListener_option2 = new OnItemSelectedListener() {
        @Override
        public void onItemSelected(int index) {
            try {
                if (mOptions3Items != null) {
                    ...
                    if (mOptions2Items.size() > opt1Select) {
                        List<T> list = mOptions2Items.get(opt1Select);
                        ...
                        List<List<T>> lists = mOptions3Items.get(opt1Select);
                        if (!ObjectJudge.isNullOrEmpty(lists)) {
                            if (lists.size() > index) {
                                List<T> ochild = lists.get(index);
                                if (mOptions3Items.size() > opt1Select) {
                                    List<List<T>> lists1 = mOptions3Items.get(opt1Select);
                                    if (!ObjectJudge.isNullOrEmpty(lists1)) {
                                        if (lists1.size() > index) {
                                            wv_option3.setAdapter(new ArrayWheelAdapter(lists1.get(index)));
                                            opt3 = opt3 >= ochild.size() - 1 ? ochild.size() - 1 : opt3;
                                            if (opt3 >= 0) {
    ...
    public void setPicker(List<T> options1Items,
        ...
        try {
            ...
            //清除空数据项
            if (!ObjectJudge.isNullOrEmpty(mOptions2Items)) {
                List oitems = new ArrayList();
                for (List<T> options2Item : mOptions2Items) {
                    if (ObjectJudge.isNullOrEmpty(options2Item)) {
                        oitems.add(options2Item);
                    }
                }
                mOptions2Items.removeAll(oitems);
            }
            if (!ObjectJudge.isNullOrEmpty(mOptions3Items)) {
                //清除空数据项
                List oitems = new ArrayList();
                for (List<List<T>> mOptions3Item : mOptions3Items) {
                    if (ObjectJudge.isNullOrEmpty(mOptions3Item)) {
                        oitems.add(mOptions3Item);
                    }
                }
                mOptions3Items.removeAll(oitems);
            }
            // 选项2
            if (!ObjectJudge.isNullOrEmpty(mOptions2Items)) {
                wv_option2.setVisibility(View.VISIBLE);
                wv_option2.setAdapter(new ArrayWheelAdapter(mOptions2Items.get(0)));// 设置显示数据
                if (!ObjectJudge.isNullOrEmpty(options2Items) && linkage) {
            ...
            if (!ObjectJudge.isNullOrEmpty(mOptions3Items)) {
                ...
                if (!ObjectJudge.isNullOrEmpty(lists)) {
                    wv_option3.setAdapter(new ArrayWheelAdapter(lists.get(0)));
                }
                if (!ObjectJudge.isNullOrEmpty(options3Items) && linkage) {
            ...
            //若后2个都为空则第一个填充屏幕
            if (ObjectJudge.isNullOrEmpty(mOptions2Items) && ObjectJudge.isNullOrEmpty(mOptions3Items)) {
                LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
                wv_option1.setLayoutParams(params);
            }
    ...
    public int[] getCurrentItems() {
        ...
        if (mOptions3Items != null && mOptions3Items.size() > 0) {
            List<List<T>> lists = mOptions3Items.get(currentItems[0]);
            currentItems[2] = wv_option3.getCurrentItem() > (lists.get(currentItems[1]).size() - 1) ? 0 : wv_option3.getCurrentItem();
        } else {
    ...
    private void itemSelected(int opt1Select, int opt2Select, int opt3Select) {
        try {
            ...
            if (!ObjectJudge.isNullOrEmpty(mOptions3Items)) {
                List<List<T>> lists = mOptions3Items.get(opt1Select);
                if (!ObjectJudge.isNullOrEmpty(lists)) {
                    wv_option3.setAdapter(new ArrayWheelAdapter(lists.get(opt2Select)));
