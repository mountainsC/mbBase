...
package com.cloud.core.drag.sortlist;
 ...
 * @author heycosmo
 */
public class BaseDragSortListView extends ListView {
    ...
    public BaseDragSortListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs,
                    R.styleable.BaseDragSortListView, 0, 0);

            mItemHeightCollapsed = Math.max(1, a.getDimensionPixelSize(
                    R.styleable.BaseDragSortListView_collapsed_height, 1));

            mTrackDragSort = a.getBoolean(
                    R.styleable.BaseDragSortListView_track_drag_sort, false);
            ...
            mFloatAlpha = a.getFloat(R.styleable.BaseDragSortListView_float_alpha, mFloatAlpha);
            mCurrFloatAlpha = mFloatAlpha;

            mDragEnabled = a.getBoolean(R.styleable.BaseDragSortListView_drag_enabled, mDragEnabled);

            mSlideRegionFrac = Math.max(0.0f,
                    Math.min(1.0f, 1.0f - a.getFloat(
                            R.styleable.BaseDragSortListView_slide_shuffle_speed,
                            0.75f)));

            mAnimate = mSlideRegionFrac > 0.0f;

            float frac = a.getFloat(
                    R.styleable.BaseDragSortListView_drag_scroll_start,
                    mDragUpScrollStartFrac);

            setDragScrollStart(frac);

            mMaxScrollSpeed = a.getFloat(
                    R.styleable.BaseDragSortListView_max_drag_scroll_speed,
                    mMaxScrollSpeed);

            removeAnimDuration = a.getInt(
                    R.styleable.BaseDragSortListView_remove_animation_duration,
                    removeAnimDuration);

            dropAnimDuration = a.getInt(
                    R.styleable.BaseDragSortListView_drop_animation_duration,
                    dropAnimDuration);

            boolean useDefault = a.getBoolean(
                    R.styleable.BaseDragSortListView_use_default_controller,
                    true);

            if (useDefault) {
                boolean removeEnabled = a.getBoolean(
                        R.styleable.BaseDragSortListView_remove_enabled,
                        false);
                int removeMode = a.getInt(
                        R.styleable.BaseDragSortListView_remove_mode,
                        DragSortController.FLING_REMOVE);
                boolean sortEnabled = a.getBoolean(
                        R.styleable.BaseDragSortListView_sort_enabled,
                        true);
                int dragInitMode = a.getInt(
                        R.styleable.BaseDragSortListView_drag_start_mode,
                        DragSortController.ON_DOWN);
                int dragHandleId = a.getResourceId(
                        R.styleable.BaseDragSortListView_drag_handle_id,
                        0);
                int flingHandleId = a.getResourceId(
                        R.styleable.BaseDragSortListView_fling_handle_id,
                        0);
                int clickRemoveId = a.getResourceId(
                        R.styleable.BaseDragSortListView_click_remove_id,
                        0);
                int bgColor = a.getColor(
                        R.styleable.BaseDragSortListView_float_background_color,
                        Color.BLACK);
    ...
    private class AdapterWrapper extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView != null) {
                ...
                child = mAdapter.getView(position, oldChild, BaseDragSortListView.this);
                if (child != oldChild) {
                ...
                child = mAdapter.getView(position, null, BaseDragSortListView.this);
    ...
    public interface FloatViewManager {
         ...
         *                      and the offset provided in {@link BaseDragSortListView#startDrag}.
     ...
     * {@link BaseDragSortListView#setDragScrollProfile(DragScrollProfile)} to
     * set custom profile.
     *
     * @author heycosmo
     */
    ...
    private class DragScroller implements Runnable {
        ...
        public void stopScrolling(boolean now) {
            if (now) {
                BaseDragSortListView.this.removeCallbacks(this);
                mScrolling = false;
            } else {
                mAbort = true;
            }
        ...
        public void run() {
            ...
            BaseDragSortListView.this.layoutChildren();
            invalidate();
