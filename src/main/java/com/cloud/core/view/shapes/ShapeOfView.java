...
public class ShapeOfView extends FrameLayout {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            if (attributes.hasValue(R.styleable.ShapeOfView_sov_clip_drawable)) {
                final int resourceId = attributes.getResourceId(R.styleable.ShapeOfView_sov_clip_drawable, -1);
                if (-1 != resourceId) {
    ...
    public void setDrawable(int redId) {
        setDrawable(getResources().getDrawable(redId));
    }
    ...
    private void calculateLayout(int width, int height) {
        ...
        if (clipManager != null) {
            if (width > 0 && height > 0) {
                ...
                if (Build.VERSION.SDK_INT >= 21 && getElevation() > 0f) {
                    try {
