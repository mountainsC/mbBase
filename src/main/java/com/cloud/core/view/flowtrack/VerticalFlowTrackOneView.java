...
public class VerticalFlowTrackOneView extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        buttomOffset = (int) a.getDimension(R.styleable.VerticalFlowTrackOneView_vftov_buttomOffset, PixelUtils.dip2px(context, 30));
        a.recycle();
    ...
    private void drawNode(FlowTrackOneItem flowTrackOneItem, int position) {
        ...
        flowTrackViewUtils.setContentHeight(flowTrackViewUtils.getContentHeight() + nodeSize);
    }
    ...
    private void drawNodeLine() {
        try {
            if (!ObjectJudge.isNullOrEmpty(nodeKeys)) {
                ...
                lineItem.setStopY(stopy + PixelUtils.dip2px(getContext(), 160));
    ...
    private int drawTimeText(FlowTrackOneItem flowTrackOneItem, TextPaint textPaint, int position) {
        ...
        if (position == 0) {
            ...
            canvasParams.setDy(currMaxDy + perFlowItemSpacing);
        }
        currMaxDy = canvasParams.getDy() + (int) (textsl.getLineCount() * timeTextSize);
        canvasParams.setStaticLayout(textsl);
        int count = flowTrackViewUtils.getKeys().size() + 1;
        flowTrackViewUtils.getKeys().add(count);
        timeKeys.add(count);
