...
class DynamicViewGroup extends ViewGroup {
    ...
    private OnMeasureDynamicViewSizeListener onMeasureDynamicViewSizeListener = null;
    private float maxViewWidth = 0;
    private float maxViewHeight = 0;

    public void setOnMeasureDynamicViewSizeListener(OnMeasureDynamicViewSizeListener listener) {
        this.onMeasureDynamicViewSizeListener = listener;
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mOrientation = typedArray.getInt(R.styleable.DynamicViewGroup_dvg_orientation, HORIZONTAL);
        mGravity = typedArray.getInt(R.styleable.DynamicViewGroup_dvg_gravity, GRAVITY_LEFT);
        mMaxColumnNum = typedArray.getInt(R.styleable.DynamicViewGroup_dvg_maxColumnNum, NUM_NOT_SET);
        mMaxLineNum = typedArray.getInt(R.styleable.DynamicViewGroup_dvg_maxLineNum, NUM_NOT_SET);
        mHorizontalSpacing = (int) (typedArray.getDimension(R.styleable.DynamicViewGroup_dvg_horizontalSpacing, 0.0f));
        mVerticalSpacing = (int) (typedArray.getDimension(R.styleable.DynamicViewGroup_dvg_verticalSpacing, 0.0f));
    }

    public void setMaxViewWidth(float maxViewWidth) {
        this.maxViewWidth = maxViewWidth;
    }

    public void setMaxViewHeight(float maxViewHeight) {
    ...
    private void measureDependOnMode(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.AT_MOST && heightMode == MeasureSpec.AT_MOST) { // 宽高都是wrap_content
            ...
            if (heightMode == MeasureSpec.UNSPECIFIED && heightSize == 0) {
                heightSize = GlobalUtils.getScreenHeight(getContext()) * 4 / 5;
                heightBeMax = false;
            }
            ...
            if (widthMode == MeasureSpec.UNSPECIFIED && widthSize == 0) {
                widthSize = GlobalUtils.getScreenWidth(getContext());
                widthBeMax = false;
            }
    ...
    private void calculateSizeAndSetMeasuredDimensionDependOnMode(int maxWidth, boolean widthBeMax, int maxHeight, boolean heightBeMax) {
        ...
        if (onMeasureDynamicViewSizeListener != null) {
            onMeasureDynamicViewSizeListener.onMeasureDynamicViewSize(resultSize.getResultWidth(), resultSize.getResultHeight());
        }
    ...
    private ResultSize getMeasureResultSize(int maxWidth, boolean widthBeMax, int maxHeight, boolean heightBeMax) {
        ...
        for (int i = 0; i < calculateSize.getChildCount(); i++) {
            ...
            if (mOrientation == HORIZONTAL) {
                calculateSize = calculateForHorizontal(calculateSize, i, isNewLineOrNewColumnByChildViewIndex(mChildViewInThisLineOrColumnForMeasure.size()));
            } else {
                calculateSize = calculateForVertical(calculateSize, i, isNewLineOrNewColumnByChildViewIndex(mChildViewInThisLineOrColumnForMeasure.size()));
            }
        }
        // 处理额外多添加的横向间距和竖向
        ...
        //当前子视图个数大于1时设置宽度和高度边界
        if (calculateSize.getChildCount() > 1) {
            if (widthBeMax) {
