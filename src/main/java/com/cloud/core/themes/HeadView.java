...
public class HeadView extends FrameLayout {
    ...
    private void bindLmrView(View view, TypedArray array) {
        try {
            ...
            if (returnIb.getVisibility() == VISIBLE) {
                returnIb.measure(0, 0);
                if (returnIb.getMeasuredWidth() > leftWidth) {
                    leftWidth = returnIb.getMeasuredWidth();
                }
            }
            if (cancelTv.getVisibility() == VISIBLE) {
                cancelTv.measure(0, 0);
                if (cancelTv.getMeasuredWidth() > leftWidth) {
                    leftWidth = cancelTv.getMeasuredWidth();
                }
            }
            if (confirmTextTv.getVisibility() == VISIBLE) {
                confirmTextTv.measure(0, 0);
                if (confirmTextTv.getMeasuredWidth() > rightWidth) {
                    rightWidth = confirmTextTv.getMeasuredWidth();
                }
            }
            if (confirmTextTv2.getVisibility() == VISIBLE) {
                confirmTextTv2.measure(0, 0);
                if (confirmTextTv2.getMeasuredWidth() > rightWidth) {
                    rightWidth = confirmTextTv2.getMeasuredWidth();
                }
            }
            int max = Math.max(leftWidth, rightWidth);
            RelativeLayout.LayoutParams tvParams = (RelativeLayout.LayoutParams) tv.getLayoutParams();
            tvParams.setMargins(max, tvParams.topMargin, max, tvParams.bottomMargin);
        } catch (Exception e) {
